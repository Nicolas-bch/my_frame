import re
import numpy as np
from utils.GeneralNewsExtractor.utils import iter_node
from utils.GeneralNewsExtractor.defaults import USELESS_TAG
...
class ContentExtractor:
    ...
    def extract(self, selector):
        ...
        result = sorted(self.node_info.items(), key=lambda x: x[1]['score'],
                        reverse=True)
        return result
    ...
    def calc_text_density(self, element):
        ...
        :return:
        """
        ti_text = '\n'.join(self.get_all_text_of_element(element))  # 换行用\n
        ti = len(ti_text)
    ...
    def calc_new_score(self, std):
        ...
        for node_hash, node_info in self.node_info.items():
            score = np.log(std) * node_info['density'] * np.log10(
                node_info['text_tag_count'] + 2) * np.log(node_info['sbdi'])
